...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setProducts(
        dummyProducts.filter((p) => category === "" || p.category === category),
      );
  ...
  const updateFilter = (key: string, value: string) => {
    ...
    // Example: ?category=fruits&page=2
    const newParams = new URLSearchParams(searchParams);

    // If the new value exists (not "", null, or undefined)
    if (value !== "" && value !== null && value !== undefined) {
      // Add the parameter if it doesn't exist,
      // or update it if it already exists.
      // Example:
      // key = "category", value = "vegetables"
      // Result: ?category=vegetables&page=2
      newParams.set(key, value);
    } else {
      // If the value is empty, remove the parameter completely.
      // Example:
      // key = "category"
      // Result: ?page=2
    ...
    // If we're changing any filter other than "page",
    // reset the pagination by removing the page parameter.
    // Example:
    // Before: ?category=fruits&page=3
    // After:  ?category=fruits
    if (key !== "page") {
  ...
  // Removes all filters and optionally keeps the page reset.
  const clearFilters = () => {
    setSearchParams({});
  };
  ...
  return (
    <div className="min-h-screen bg-app-cream">
          ...
          <main className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex flex-col lg:items-center gap-3">
                ...
                >
                  <SlidersHorizontal className="size-4" onClick={()=>setMobileFiltersOpen(true)}/>
                </button>
                {/* sort */}
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-sm text-app-text mb-4">
                  Try adjusting you filters or search terms
                </p>
                <button
      ...
      {mobileFiltersOpen && (
        <>
        <div className="fixed inset-0 bg-black/40 z-50 overflow-hidden"  onClick={()=> setMobileFiltersOpen(false)}>
        <div className="fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-2xl max-h-[70vh] overflow-y-auto animate-slide-in-up">
