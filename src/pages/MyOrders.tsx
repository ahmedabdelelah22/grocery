...
const MyOrders = () => {
  ...
  }, [activeTab, searchParams, clearCart, setSearchParams, fetchOrders]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <h2 className="text-lg font-medium text-app-green mb-2">No orders</h2>
            <p className="text-sm text-app-text-light mb-4">Start shopping to see your orders here</p>
            <Link className="inline-flex px-4 py-2 bg-app-green text-white text-sm rounded-lg" to="/products">
              Start shopping
            </Link>
          </div>
        ) : (
          <div className="space-y-4">
            {filteredOrders.map((order) => (
              <Link
                ...
                to={`/orders/${order._id}`}
                className="block
                  max-w-4xl bg-white rounded-2xl p-5 hover:shadow transition-all"
              >
